...
function doRectanglesOverlap(rect1, rect2) {
  return !(rect1.left >= rect2.left + rect2.width || rect2.left >= rect1.width + rect1.left
...
function getDigitalRoot(num) {
  ...
  return s > 10 ? getDigitalRoot(s) : s;
}
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < position.length; i += 1) {
    for (let j = 0; j < 1; j += 1) {
      if (position[i][j] === position[i][j + 1] && position[i][j + 1] === position[i][j + 2]
        && position[i][j]) {
        return position[i][j];
      }
      if (position[j][i] === position[j + 1][i] && position[j + 1][i] === position[j + 2][i]
        && position[j][i]) {
        return position[j][i];
      }
    }
  ...
  || (position[0][2] === position[1][1] && position[1][1] === position[2][0] && position[1][1])) {
    return position[1][1];
  }
  return undefined;
}
